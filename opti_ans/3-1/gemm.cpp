...
#pragma omp parallel for num_threads(tn)
    for (i = 0; i < ni / 2; i++) {
      for (j = 0; j < nj - step; j += step) {
        ...
        c_vec = _mm512_mul_pd(c_vec, _mm512_set1_pd(beta));
    ...
    double sum = 0;
#pragma omp parallel for num_threads(tn)
    for (i = 0; i < ni / (astep * 2); i++) {
      for (k = 0; k < nk / astep; k++) {
        for (j = 0; j < nj / astep; j++) {
          int x, y, z;
          for (x = i * astep; x < (i + 1) * astep; x++) {
            for (y = k * astep; y < (k + 1) * astep; y++) {
              for (z = j * astep; z < (j + 1) * astep; z++) {
                C[x][z] += alpha * A[x][y] * B[y][z];
              }
            }
          }
        }
